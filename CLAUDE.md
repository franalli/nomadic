...
- **Active work:** Dead code cleanup (~1,600 lines removed across frontend+backend, including `useDateRangeSelector.ts`, `confirm-dialog.tsx`). New LLM provider deps (langchain-google-genai, langchain-anthropic). Input gate system: 5 pre-routing validators (DateGate, DurationGate, TravelerGate, BudgetGate, DestinationGate) with blocking/warning severity, short-circuit to synthesizer on violations, gate-blocked LLM explanation with fallback. Multi-destination detection via RouterOutput `multi_destination_detected` field + DestinationGate warning. Router: same-month date change skips specialist re-run, exploration→soft_transition auto-upgrade when dates collected, Tier 2 prefetch fires regardless of Tier 1, settings merge regex fallback, reset suppression fix, \_is_plan_active() helper, input gate integration in two phases. Specialist: activity count scaling by trip length (2-3 short, 3-5 medium, up to 7 long). Logistics: selective Tier 1 activity backfill for free days (replaces full suppression on long trips). Builder: Phase 5.6 logistics backfill fallback for pure Tier 1 trips, Phase D capacity-based co-scheduling with Tier 2 reserve, pinned tile Pass 0, altitude acclimatization dynamic placement, activity input/placed tracking + drop ratio. Guard: DATE_IN_PAST, MULTI_SPECIALIST_CAPACITY_EXCEEDED, cross-domain capacity gate, drop-ratio-aware suppression. Synthesizer: structured suggestion chips with PILL_ACTION_MAP action routing (open_pill/send_message), gate-blocked context injection, input gate violation/warning synthesis. Response envelope `suggestion_chips` field. SuggestionChip schema (action_type, action_target). TurnMeta: input_gate_violations/warnings fields. Cache dead code removal (reset_stats, cleanup_expired, get_cache_handle).
- **Known broken:** none
