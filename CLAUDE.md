...
- **Active work:** Dead code cleanup (~2,200 lines removed: `base_cache.py`, `S3ItineraryView.tsx`, `CollapsedSetupSummary.tsx`, dead cache/telemetry/validation functions, `collapseSetupMessages`, `useCartActions`, `CONFLICT_COPY`). New `backend/app/utils/tile_utils.py` shared tile flattener. Input gate system: 5 pre-routing validators with blocking/warning severity, gate-blocked LLM explanation (`router_extracted_fields` set before gates). Multi-destination detection. Synthesizer: token limits (200/250/350), prompt rewrite ("Expedition Leader" voice, terse settings acks, post-plan updates ≤15 words), structured suggestion chips with PILL_ACTION_MAP action routing, ChatPanel sheet-opening chip handler, builder conflict surfacing, date delta context, anti-repetition turn numbering, specialist_update history depth 8. StrategySection: new fields (editorial_one_liner, vibe_trio, hero_image, travel_intelligence). Specialist: activity count scaling by trip length (default cap 3), day_preference cache key (`dpref`), Tier 1-only pref filter, month:bucket invalidation key (not exact dates), incremental parallel_llm_results merge, fire-and-forget Unsplash prefetch, day preference scoping to mentioned categories. Logistics: selective Tier 1 activity backfill (co-scheduling formula), separate hotel/activity cache hashes (prevent cross-busting). Builder: Phase 5.6 logistics backfill fallback, conflict details surfaced to metadata. Fill-day: Tier 1/Tier 2 separation, specialist tile reuse (round-robin), registry-driven constraint validation (`validate_fill_day_placement` with rejection responses), cross-domain exclusions from registry, real-block guard in TimelineThread. PATCH endpoint: last-writer-wins (version drift reload + diff guard). Unsplash: async lock, non-blocking `get_image_url_sync()`, `get_cached_image_url()` for experience tiles. Preference PATCH debounced 500ms, usePreferenceAutoRegen debounced 1.5s + streaming gate. Architect: secondary extraction skip guard (fields already present). `isBookableActivityTile` filter. Activity settings day_preferences merge guard. PlanHeader generation pulse animation. Suggestion pool: "change destination" chip removed. Synthesizer `change_type` signal routing (STEPPER_RERUN, SETTINGS_CHANGE, NEW_CATEGORY, DATE_CHANGE, PLAN_UPDATE) for specialist_update responses; prompt split into specialist_update + planning branches with per-type templates.
- **Known broken:** none
