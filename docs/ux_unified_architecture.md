...
| `TripSummaryPills` | Unified command bar for trip inputs plus module actions: destination/origin/dates/travelers/budget/activities plus inline command segments for Flights, Stays, and Destination Travel Advice (with pending/badge state). On mobile overflow, the actual horizontal scroller owns a right-edge fade that disappears once the reel reaches scroll end. |
| `NomadicLanding` | Desktop shell for the main planning route. Owns reset CTA, account popover wiring (`useUserStore` login/logout/fetchTrips), and recent-trip resume actions while still delegating plan rendering to `StrategyStageRenderer`. The reset action is now always visible in the right-side control zone, even when signed out. |
| `MobileModeHeader` | Mobile top bar for the planning route. Hosts overflow actions for reset, share, PDF export, sign in/out, and recent-trip resume without changing the core swipe-layout topology. On Plan page scroll collapse it renders a secondary condensed summary bar beneath the fixed header and suppresses the floating status pill; entering Plan also blurs the active input/textarea to dismiss the keyboard. Signed-out users now get a reset action inside the overflow menu; signed-in users retain the dedicated reset affordance. |
