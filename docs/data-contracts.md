...
> **Source files:** `backend/app/schemas.py`, `backend/app/main.py`, `frontend/state/documentStore.ts`, `frontend/lib/api.ts`, `frontend/lib/api-document.ts`, `frontend/lib/api-streaming.ts`, `frontend/types/plan-envelope.ts`
...
| Method | Path                              | Purpose                                | Request                                                     | Response                                                                                                                      |
| ------ | --------------------------------- | -------------------------------------- | ----------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| POST   | `/api/analytics/event`            | Fire-and-forget funnel event tracking  | `TrackEventRequest{event_type (max 48), destination?, metadata?}` | `{status: "ok"}`                                                                                                              |
...
| GET    | `/health`        | Health check (120/min rate limit). Returns `status: "ok"` or `"degraded"`, env, build hashes, and `db: {ok, pool: {size, checkedin, checkedout, overflow}}` | JSON                   |
...
| `node_status` | `{type: "node_status", data: {node, status: "started"\|"completed", label, icon_key, estimated_duration_ms}}` | Agent tool-call progress, derived from the `updates` stream mode by `run_agent_turn_streaming` (started on tool dispatch, completed on the ToolMessage). `node` is one of the 6 tool names — `extract_trip_fields`, `get_specialist_advice`, `search_tiles`, `get_local_intel`, `validate_plan`, `build_itinerary` — plus `response` for the terminal model turn. A turn with no tool calls (greeting/question) emits only the terminal `response` node_status. |
| `complete`    | `{type: "complete", data: {document, session_state, version, response_degraded?, response_error_type?, ...}}` | Full response envelope. `document` includes `day_cards`, `itinerary_overview`, `itinerary_assumptions`, `hotel_filter_cascaded`, `suggested_responses`, `suggested_response_meta`, `suggestion_chips` (structured chips with action routing), `constraints_validated`, `constraint_violations`, `tiles_replaced`, `trip_cost_estimate`, and `ack_status`/`ack_updates`/`applied_updates`. When `response_degraded=true`, the LLM response generation failed and a fallback message was used; `response_error_type` indicates the cause (`"rate_limit"` etc.). When `day_cards` are present: `plan_view_state=S3_ITINERARY_READY` if conflict count is 0, `S3_EDITING` if conflicts exist, and `S3_PARTIAL_CONFLICT` on partial-failure path with returned day cards. When no concrete cards are produced: `S3_BLOCKED`. **`observability`** subfield currently emits `tokens`, `today_iso`, and `ready_to_generate_now`; it does not currently persist separate preview counters from `specialist_preview` partials. Optional frontend fields such as `extraction_confidence`, `short_circuit_type`, `llm_calls_made`, `cache_hits`, and `confidence_routing` are reserved and not populated by backend today. |
| `partial`     | `{type: "partial", data: {kind, payload, ...}}` | Progressive render before `complete`. Pushed as `custom` stream events ( `{kind, payload}` ) by the tool layer and multiplexed into `partial` by `run_agent_turn_streaming`: `extract_trip_fields` (`trip_inputs`), `get_specialist_advice` (`specialist_preview` carrying lightweight topic snippets plus the current authoritative `strategy_sections`, followed by a compatibility `strategy_sections` partial that repeats that same section set and is deferred by one animation frame on the client), `search_tiles` (`tiles`, ID-keyed map matching `document.tiles`; optionally followed by `date_flex_suggestion` with `{cheapest_date, cheapest_price, requested_date, requested_price, savings, savings_pct, currency, nearby_prices}` when Aviasales grouped_prices fallback finds a ≥10% cheaper flight within ±3 days and the trip is >7 days out — ephemeral, not persisted), and `build_itinerary` itinerary/card-conflict updates (`day_cards`, optionally with `context.plan_view_state` / `itinerary_overview` / `itinerary_assumptions` / `constraint_violations` / `warnings`). Enrichment data (partner pricing, deeplinks, Google Places) is now awaited before envelope construction, so it lands in the `complete` event directly — the backend no longer emits post-response `tile_enrichment` partials. Tile-bearing partials may also carry `tiles_replaced=true` so the frontend can replace activity inventory immediately instead of waiting for the final envelope. Frontend merges via `store.mergeEnvelope(envelope, generation)` so stale events from prior send cycles are ignored. Errors must not crash stream -- wrapped defensively. `complete` event reconciles any differences. |
| `feasibility_warning` | `{type: "feasibility_warning", data: {topic, status, reason, alternative}}` | Feasibility signal for geographic-constrained specialists, derived by `run_agent_turn_streaming` from a `get_specialist_advice` ToolMessage flagging `feasibility_status="infeasible"`. Forwarded by `generate_sse()` as a public SSE event. Frontend handles via `onFeasibilityWarning` callback for toast display. |
| `heartbeat`   | `{type: "heartbeat"}`                                                                   | Connection keepalive emitted every 5s by `generate_sse()`. Frontend uses it to reset the SSE watchdog timer; no state mutation. |
...
| **Light-read**           | `document` (GET+PATCH), `chat`, `chat/last`, `session`, `tiles/click`, `analytics/event`, `document/validate-arrangement` | 60/min                                          |
...
| `geocode_cache_ttl_hours`        | 24                   | `GEOCODE_CACHE_TTL_HOURS`      | L2 geocode cache TTL                                        |
| `google_places_enrichment_max_parallel` | 4             | `GOOGLE_PLACES_ENRICHMENT_MAX_PARALLEL` | Max concurrent Google Places enrichment requests            |
| `google_places_enrichment_retry_attempts` | 2           | `GOOGLE_PLACES_ENRICHMENT_RETRY_ATTEMPTS` | Retry attempts for failed enrichment requests               |
| `google_places_enrichment_retry_base_ms` | 250          | `GOOGLE_PLACES_ENRICHMENT_RETRY_BASE_MS` | Base delay (ms) for enrichment retry backoff                |
...
`turn_meta` uses the `_merge_turn_meta` reducer: `tools_called`, `turn_steps`, and `partial_failures` are concatenated (no dedup) so parallel same-tool calls accumulate instead of clobbering; `tool_call_count` is derived from `len(tools_called)` after merge; all other keys (`validation_result`, `builder_result`, ...) are right-wins. Each tool returns only its DELTA for the concatenated keys (e.g. `{"tools_called": [name]}`). `strategy_sections` dedupes by `specialist_type` (right wins), `constraints` by `constraint_id`; other shared dicts are shallow-merged right-wins.
...
- `TripEvent` stores lightweight analytics events for trip planning funnel tracking: `session_id`, `event_type` (max 48), optional `destination` (max 200), optional `metadata` (JSON). Indexed on `session_id` and `event_type`.
...
- **Session cleanup:** A background task in `lifespan.py` (`_cleanup_expired_sessions`) runs every 6 hours, deleting sessions inactive for 14+ days (100 per batch) along with their `ChatMessage`, `PlanDocument`, and `TripContext` rows.
...
PlanDocumentData
  ...
  |-- hotel_filter_cascaded (NOT on Pydantic model -- injected at envelope assembly time by coordinator)
...
| `ConstraintValidation`        | Trip DNA badge result: `constraint_id`, `rule`, `status`, `specialist`, `label?`                                                                                                                                                                                                                                                                                                                                  |
| `ConstraintViolation`         | Constraint violation: `code`, `message`, `severity`, `category`, `rule?`, `suggested_action?`                                                                                                                                                                                                                                                                                                                     |
...
Backend still emits S-prefix states (computed in `_build_envelope`):
...
| Progressive preview | `_specialistPreview` (`SpecialistPreviewActivity[] \| null`), `_partialVersion` (`number \| null`) |
| Send-cycle nonce    | `messageSendNonce` (number) -- incremented on each user send to gate stale completion/scroll side-effects |
...
| `setSpecialistPreview()`                                           | Ingest specialist preview payload: stores activity previews in `_specialistPreview`, optionally merges `strategy_sections` from preview into document |
| `setPartialDayCards()`                                             | Merge progressive `day_cards` partial (with tiles, `strategy_sections`, `plan_view_state`, itinerary context) via `mergeEnvelope()` |
| `mergeTileEnrichment()`                                            | Merge post-response tile enrichment partial (conditional `day_cards`/`tiles` merge based on change flags, plus itinerary context). Vestigial — backend no longer emits `tile_enrichment` partials (enrichment data is now in the `complete` envelope). |
| `setDateFlexSuggestion()`                                          | Set/clear ephemeral `dateFlexSuggestion` (from `date_flex_suggestion` SSE partial). Cleared on each new turn. Drives `DateFlexChip` in chat suggestions. |
| `bumpMessageSendNonce()`                                           | Increment `messageSendNonce` on each user send (gates stale enrichment poller and scroll side-effects)                                  |
...
- **Pre-document partial bootstrap:** `mergeEnvelope()` creates a minimal document shell when partials arrive before `setFromPlanResponse()` has initialized the document, so progressive renders can display strategy sections and day cards without waiting for the full envelope
...
Source: `frontend/lib/api.ts` (core `apiFetch`, CSRF, retry), `frontend/lib/api-document.ts` (document mutations), `frontend/lib/api-streaming.ts` (SSE streaming, browse, enrichment), `frontend/lib/sharedTripApi.ts`
...
| `streamGraphPlan()` *(api-streaming)* | POST `/api/graph_plan/stream` | SSE streaming (onToken, onComplete, onError, onNodeStatus, onPartial, onFeasibilityWarning callbacks). `onPartial` receives `SSEPartialData`, a discriminated union keyed by `kind`: `'strategy_sections' \| 'tiles' \| 'trip_inputs' \| 'day_cards' \| 'specialist_preview' \| 'tile_enrichment' \| 'date_flex_suggestion'`, each with strongly-typed payloads (the `tile_enrichment` variant remains in the union but is no longer emitted by the backend). Returns abort fn. Non-2xx responses preserve backend `detail` text in thrown errors when available. Exported types: `SSEPartialData`, `SSEPartialContext`, `SpecialistPreviewSection`, `SpecialistPreviewActivity`, `SpecialistPreviewPayload`, `DayCardsPartialPayload`. Note: `TileEnrichmentPayload` type still exists in frontend but the backend no longer emits `tile_enrichment` partials. |
...
| `refreshTiles()` *(api-document)* | POST `/api/tiles/refresh` | Refresh tiles for branch (uses `fetchWithRetry`, 2 retries, 500ms base delay)                                                                                                                                                 |
| `fillDay()` *(api-document)* | POST `/api/document/fill-day`   | Generate activity tiles for a free day. Returns `tiles` map for store merge. Response may include `rejected: true` with `rejection_reason`, `rejection_code`, `rejection_suggestion` when Tier 1 constraint validation fails. Non-2xx errors preserve backend `detail` text in thrown error messages (used for 429/rejection toasts). |
| `validateArrangement()` *(api-document)* | POST `/api/document/validate-arrangement` | Check proposed block moves against constraints. Returns `{valid, violations[]}`. No LLM, target <50ms. |
| `applyArrangement()` *(api-document)* | POST `/api/document/apply-arrangement` | Validate + persist block moves. Throws `'VERSION_CONFLICT'` on 409. On success, returns `{valid, violations, day_cards, version}` -- caller must `mergeEnvelope({day_cards})` and `setState({version})` separately. |
| `removeBlock()` *(api-document)* | POST `/api/document/remove-block` | Remove a single block. Throws `'VERSION_CONFLICT'` on 409. Returns `{day_number, day_card, version, removed_block_id}`. |
| `browseActivities()` *(api-streaming)* | POST `/api/activities/browse` | Browse destination activities for a destination. Params: `{destination, dayNumber?, date?, hotelLocation?, categories?}`. Backend fetch is Viator-first, then GYG supplement/dedupe, then Google Places fallback/supplement; if Maps is unavailable, partner-only live results may still be returned. Returns `{tiles: BrowseTile[], total: number}` (`api.ts` client typing; backend may also include `source: "stashed" \| "places"` and keeps `"places"` as the live-provider label). |
| `getSpecialistEnrichment()` *(api-streaming)* | GET `/api/specialist/{sectionId}/enrichment` | Fetch Phase B enrichment status for a specialist section. `local_expert` requests long-poll for up to 6s (500ms cadence) before the backend falls back to `202 {status:'pending', retry_after_ms:1500}`. Returns `{status: 'ready'\|'pending'\|'failed', section_id, data?, error_code?, retry_after_ms?}` or `null` (404). |
| `fetchSharedTrip()` *(api-streaming)* | GET `/api/shared/{slug}` | Public shared-trip client fetch used by `/trip/[slug]` fallback UI. Delegates to `sharedTripApi.ts`, which also exports the server-side `fetchSharedTripServer()` helper used by the RSC route. Uses `credentials: 'omit'`; throws user-facing `404`/`410` errors for missing or expired shares. |
| `trackDeeplinkClick()` *(api-streaming)* | POST `/api/tiles/click` | Fire-and-forget analytics ping for partner deeplink opens. Uses `apiFetch()` and intentionally swallows client-side failures. |
| `trackEvent()` (from `analytics.ts`) | POST `/api/analytics/event` | Fire-and-forget funnel event tracking with `keepalive` for page-unload resilience. Events: `session_start`, `trip_created`, `itinerary_generated`, `trip_shared`, `preference_hearted`, `hotel_clicked`, `flight_clicked`, `activity_clicked`. |
...
- `api.ts` was split into three modules: `api.ts` (core `apiFetch`, CSRF, retry), `api-document.ts` (document mutations: fill-day, arrangement, block removal, tile refresh), `api-streaming.ts` (SSE streaming, browse activities, specialist enrichment, shared trips, deeplink tracking).
