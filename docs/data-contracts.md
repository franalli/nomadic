...
| `node_status` | `{type: "node_status", data: {node, status: "started"\|"completed", label, icon_key, estimated_duration_ms}}` | Coordinator step progress mapped to legacy node/tool names. Fields include `node`, `status`, `label`, `icon_key`, `estimated_duration_ms`, and optional `topic`/`stage` now populated by the coordinator for parallel step tracking. Common values: `extract_trip_fields`, `get_specialist_advice`, `get_local_intel`, `search_tiles`, `build_itinerary`, `response`. `SHORT_CIRCUIT` emits no `node_status` event. |
