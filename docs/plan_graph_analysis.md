...
- Single-day fill-day entries use the same table with `cache_type='experience_single'`; cache-clear paths remove both `experience` and `experience_single`
...
- TTL: 24 hours
- Storage: JSONB array of tile dicts
- L2 write failures roll back the SQLAlchemy session (`OperationalError` and generic exception paths) to avoid poisoned transactions
...
| `/api/admin/clear-all-caches`       | POST   | Comprehensive clear of ALL caches (includes L1+L2 for specialist, experience, tile, router) |
...
**Amadeus singleton fallback:** `AmadeusFlightProvider` and `AmadeusHotelProvider` read `AmadeusClient._instance` in sync contexts. If startup has not initialized it yet, they create a client and register it back to `_instance` for shared lifecycle cleanup.
