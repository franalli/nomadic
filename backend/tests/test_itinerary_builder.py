...
class TestDivingHikingCombo:
    """Test diving + hiking combo respects no-fly buffer while allowing light activities."""

    def test_no_fly_buffer_injected(
        self, builder: ItineraryBuilder, basic_input: ItineraryBuilderInput
    ):
        """Diving trips get 24h no-fly buffer on day before departure."""
        result = builder.build(basic_input)

        assert result.success

        # Day before departure should be no-fly
        second_to_last_day = result.day_cards[-2]
        assert second_to_last_day.label == "No-Fly Day"

        # Should have buffer block
        buffer_blocks = [b for b in second_to_last_day.blocks if b.buffer_type == "no_fly"]
        assert len(buffer_blocks) == 1
        assert buffer_blocks[0].buffer_reason is not None
        assert "PADI" in buffer_blocks[0].buffer_reason

    def test_no_fly_day_excludes_diving(
        self, builder: ItineraryBuilder, multi_specialist_input: ItineraryBuilderInput
    ):
        """No diving activities should be scheduled on no-fly day."""
        result = builder.build(multi_specialist_input)
        ...
        assert no_fly_day is not None

        # No diving activities on no-fly day (excluding the buffer block itself)
        diving_on_buffer = [
            b for b in no_fly_day.blocks if b.specialist_type == "diving" and not b.is_buffer
        ]
        assert len(diving_on_buffer) == 0

    def test_light_activities_allowed_on_no_fly_day(self, builder: ItineraryBuilder):
        """Light/hiking activities can be scheduled on no-fly day."""
        # The buffer block itself indicates light activities are OK
        input_data = ItineraryBuilderInput(
            start_date="2024-03-15",
            end_date="2024-03-19",  # 5 days (arrival + 1 dive day + 1 spare + buffer + departure)
            strategy_sections=[
                {
                    "specialist_type": "diving",
                    "content_added": [
                        {"title": "Single Dive", "duration_hours": 3.0},
                    ],
                    "constraints_applied": [
                        {"rule": "min_24h_buffer_after_dive"},
                    ],
