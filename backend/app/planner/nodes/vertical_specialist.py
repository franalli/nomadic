...
from app.planner.schemas.coordinator_schemas import SpecialistConstraintOutput
...
# Canonical definition lives in coordinator_schemas.SpecialistConstraintOutput.
# Alias kept for backwards-compat with tests and existing references.
LLMConstraint = SpecialistConstraintOutput
...
async def _merge_specialist_into_state(
    ...
    for block in output.content_blocks:
        ...
        if not image_url:
            # Last resort: try with just destination + title (broader Unsplash query)
            image_url = get_activity_image("", state.trip_plan.destination or "", block.title)
