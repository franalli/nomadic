...
class ItineraryBuilder:
    ...
    def _place_experience_tiles(
        ...
        # Fallback: logistics backfill tiles (curated activities from selective_activity_backfill)
        # Only used when no experience tiles exist (pure Tier 1 trips)
        if not experience_tiles:
            logistics_tiles = [
                t
                for _, t in tiles.items()
                if isinstance(t, dict)
                and t.get("source_agent") == "logistics_node"
