          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2 overflow-x-auto no-scrollbar -mx-4 px-4 pb-2">
              {section.destination_gallery.filter(img => img.image_url).map((img) => (
                <div key={img.image_url} className="relative w-32 h-24 rounded-lg overflow-hidden shrink-0">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              {section.content_added.map((item, i) => (
                <div key={`${item.title}-${i}`} className={DS.infoBox.container}>
    ...
    <>
      ...
      {constraints.length > 0 && (
        <div className="space-y-4"><h4 className={DS.text.label}>Applied Constraints</h4>
          <div className="space-y-2">{constraints.map((c) => (
      ...
      {section.principles && section.principles.length > 0 && (
        <div className="space-y-4"><h4 className={DS.text.label}>Key Principles</h4>
          <div className="space-y-2">{section.principles.map((p, i) => (
            <div key={`${p}-${i}`} className="flex items-start gap-2"><Sparkles className="w-4 h-4 text-emerald-500 mt-0.5 shrink-0" /><span className="text-sm text-zinc-700 dark:text-zinc-300">{p}</span></div>
          ))}</div>
        </div>
      )}
      {section.content_added && section.content_added.length > 0 && (
        <div className="space-y-4"><h4 className={DS.text.label}>Expert Recommendations</h4>
          <div className="space-y-2">{section.content_added.map((item, i) => (
            <div key={`${item.title}-${i}`} className={DS.infoBox.container}><div className="flex gap-2">
