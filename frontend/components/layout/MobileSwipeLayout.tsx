...
function MobileSwipeLayoutInner({ chatContent, planContent, planTabEnabled = false }: MobileSwipeLayoutProps) {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      {/* Tab bar */}
      <div className="mx-3 my-1.5 flex items-center rounded-full border border-zinc-200 bg-zinc-100/80 p-0.5 dark:border-white/[0.08] dark:bg-white/[0.06]">
        <button
          type="button"
          onClick={() => goToPage(0)}
          className={cn(
            'min-h-10 flex-1 rounded-full px-3 py-1.5 text-xs font-semibold leading-none transition-all duration-150',
            activePage === 0
        ...
        >
          ...
          {planTabEnabled && hasNewPlanContent && activePage !== 1 && (
            <span className="absolute right-3 top-1.5 h-2 w-2 rounded-full bg-emerald-500 ring-2 ring-white dark:ring-zinc-950 animate-pulse" />
          )}
        </button>
      </div>
