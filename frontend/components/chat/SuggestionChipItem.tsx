...
/** SuggestionChipItem — individual chip button for ChatSuggestionChips. */
...
import type { BookingTypes, FlightSettings, SuggestionChip } from '@/types/document';
import type { SheetType } from '@/types/sheets';

import { handleSuggestionTriggerAction } from './suggestion-actions';

// ── Types & resolution logic ─────────────────────────────────────────────────

export type ResolvedChipAction = {
  actionType: SuggestionChip['action_type'];
  actionTarget: string | null;
};

const OPEN_PILL_TARGETS = new Set([
  'dates', 'origin', 'destination', 'travelers', 'budget', 'flights', 'stays', 'activities',
]);
...
const ACTION_TARGET_ALIASES: Record<string, string> = {
  activity: 'activities', activity_sheet: 'activities', date: 'dates',
  departure: 'origin', departure_city: 'origin',
  direct_flights_only: 'set_direct_flights_only', direct_only: 'set_direct_flights_only',
  flight_origin: 'origin', hotel: 'stays', hotels: 'stays', stay: 'stays',
  traveller: 'travelers', travellers: 'travelers',
};
...
function normalizeActionTarget(
  ...
  if (actionType === 'open_pill') return OPEN_PILL_TARGETS.has(canonical) ? canonical : null;
  if (actionType === 'trigger_action') return TRIGGER_ACTION_TARGETS.has(canonical) ? canonical : null;
  return canonical;
}
...
export function resolveChipAction(chip: SuggestionChip): ResolvedChipAction {
  ...
  if (chip.action_type !== 'send_message' && explicitTarget) {
    return { actionType: chip.action_type, actionTarget: explicitTarget };
  }
...
interface SuggestionChipItemProps {
  chip: SuggestionChip;
  idx: number;
  resolvedAction: ResolvedChipAction;
...
export function SuggestionChipItem({
  chip,
  idx,
  resolvedAction,
  bookingTypes,
  ...
  onOpenActivities,
  toast,
}: SuggestionChipItemProps) {
  const isCta = chip.chip_type === 'cta';
  const isPlanningTrigger =
    isCta ||
